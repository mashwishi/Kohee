...
const People = () => {
  ...
  const [userObjForFollower, setUserObjForFollower] = useState<any | null>(null);
  const [userObjFilteredFollower, setUserObjFilteredFollower] = useState<any | null>(null);

  const [userObjForVisits, setUserObjForVisits] = useState<any | null>(null);
  const [userObjFilteredVisits, setUserObjFilteredVisits] = useState<any | null>(null);
  ...
  useEffect(() => {
    ...
    async function getData() {
      ...
      try {
          ...
          setUserObjForFollower(data?.data?.users)
          setUserObjForVisits(data?.data?.users)
  ...
  useEffect(() => {
    ...
    if(userObjForFollower){
      const sortedFollowers = userObjForFollower.map((user: { followers: { aggregate: { count: any; }; }; }) => {
        return {
          user: user,
          followers: user.followers.aggregate.count
        };
      }).sort((a: { followers: number; }, b: { followers: number; }) => {
        return b.followers - a.followers;
      }).map((sortedUser: { user: any; }) => sortedUser.user);

      setUserObjFilteredFollower(sortedFollowers)
    }

    if(userObjForVisits){
      const sortedVisitors = userObjForVisits.map((user: { visits: { aggregate: { count: any; }; }; }) => {
        return {
          user: user,
          visits: user.visits.aggregate.count
        };
      }).sort((a: { visits: number; }, b: { visits: number; }) => {
        return b.visits - a.visits;
      }).map((sortedUser: { user: any; }) => sortedUser.user);

      setUserObjFilteredVisits(sortedVisitors)
    }

  }, [userObj, stateSearch, userObjForFollower, userObjForVisits])
        ...
        <div className="container px-5 py-24 mx-auto divide-y grid grid-cols-1">
          ...
          <div>
            <div className="flex flex-col text-center w-full">
              <h1 className="text-4xl font-bold title-font mb-4 text-gray-900">Discover</h1>
          ...
          <div>
            <div className="flex flex-col text-center w-full mt-4">
              <h1 className="text-lg font-medium title-font mb-4 text-gray-900">MOST FOLLOWED USERS</h1>
            </div>
            <div className="">
            
            {
                  userObjFilteredFollower?.length > 0 ?
                    <>
                      <Carousel 
                        swipeable={true}
                        draggable={true}
                      ...
                      >
                        {
                            (userObjFilteredFollower as any[]).map((i) => {
                            return (
                                  <GetUser_All 
                                  key={i.user_id}
                                  username={i.username} 
          ...
          <div>
            <div className="flex flex-col text-center w-full mt-4">
              <h1 className="text-lg font-medium title-font mb-4 text-gray-900">MOST VISITED USERS</h1>
            </div>
            <div className="">
            
            {
                  userObjFilteredVisits?.length > 0 ?
                    <>
                      <Carousel 
                        swipeable={true}
                        draggable={true}
                      ...
                      >
                        {
                            (userObjFilteredVisits as any[]).map((i) => {
                            return (
                                  <GetUser_All 
                                  key={i.user_id}
                                  username={i.username} 
